...
d3.chart("BaseChart", {
  initialize: function () {
    ...
    function invokeResizeListener() {
      chart.windowListen = d3.select(window)
      console.log(chart.base)
      chart.windowListen.on('resize', function() {    
        chart.width(parseFloat(getComputedStyle(chart.base.node().parentNode).width) - chart._margin.left - chart._margin.right);
        // chart.height(parseFloat(getComputedStyle(chart.base.node().parentNode).height) - chart._margin.top - chart._margin.bottom);
      }); 
    }

    invokeResizeListener();
